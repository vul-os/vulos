...
| **Notifications** | Do Not Disturb (**local to this browser** — silences pop-ups here; anyone signed in can toggle it, and it does not stop the box from sending Web Push to your phone — see below) and notification-sound toggles, the per-device Web Push toggle for this browser, and per-source toggles (Mail, Assistant, System, …) that stop a source being collected at all, not just silenced. There is a separate, admin-only, box-wide Do Not Disturb (`/api/notifications/dnd`) with no control in this pane — see [Notifications](USER-GUIDE.md#notifications) in the User Guide for both. |
...
| **This device** *(Android app only)* | Renders only inside the Vulos Android shell — it is absent in a normal browser. Three opt-in controls: **Background connection**, which runs an Android foreground service so the box stays reachable and notifications are not delayed by Doze; **Use Vulos as home screen**, which asks Android to make Vulos the default launcher and offers a jump back to Android's home-app settings to undo it; and **Biometric unlock**, a local preference the OS lock screen and the offline lock screen both read. |
...
| **Storage Mode** | Where this box reads and writes its objects: **This device** (default — plain files on this box's own disk, no object store, no credentials, no third-party service), **Local MinIO + CRDT sync** (a co-located MinIO becomes the source of truth and the sync layer replicates to peer instances), or **Central Tigris** (opt-in — every read/write goes straight to a hosted third-party bucket). A box installed before the local default was introduced keeps the backend it was already using; nothing is migrated for you. |
...
| **Native Pairing** | Shows this box's LAN TLS certificate fingerprint (SPKI SHA-256) and box name/address as a QR code and copyable text, for a native client to pin on first connection instead of trusting a public certificate authority — the same pairing payload `vulos -print-pairing` prints at the console. **This is preparation, not a working flow yet**: `clients/core/` implements the pin/verify/store logic, but no shell (desktop, Android, or iOS) calls it, and nothing scans *this* code — the one camera-QR entry point that ships (`Scan QR` in the setup wizard, `frontend/src/auth/Setup.tsx`, shown only when the native camera bridge is available) redeems a cluster **join code**, not a pairing payload. So nothing today can complete a pairing handshake from this pane. See [NETWORKING.md](NETWORKING.md#native-client-pairing-pinning-this-boxs-lan-certificate). |
...
| Pane | What you do there |
|---|---|
| **OS Update** | Vulos checks its release channel in the background and cryptographically verifies every manifest against the release-signing trust chain baked into the image — that check **never** downloads or stages anything on its own. This pane shows the running version, the latest verified version (flagged prominently if it's a security release), and lets you explicitly **Download & stage** it: staging downloads, verifies, and writes the release to the inactive A/B slot. Staging is admin-only **and** requires a fresh step-up elevation (`POST /api/os/update/stage`), on the same bar as key-management actions. A staged release **does** activate on the next reboot: the initramfs reads `boot-state.json` at init-bottom and boots the slot it names (OSDIST-FLIP-01), and it fails back to the running image if that file is missing, unreadable, or names a slot whose squashfs is absent. Nothing reboots the box for you — you choose when. See [ARCHITECTURE.md → OS distribution](ARCHITECTURE.md#os-distribution-bare-metal) for how far the proof of that goes, and [GETTING-STARTED.md → Upgrading](GETTING-STARTED.md#upgrading). |
