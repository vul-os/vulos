...
| Pane | What you do there |
|---|---|
| **OS Update** | Vulos checks its release channel in the background and cryptographically verifies every manifest against the release-signing trust chain baked into the image — that check **never** downloads or stages anything on its own. This pane shows the running version, the latest verified version (flagged prominently if it's a security release), and lets you explicitly **Download & stage** it: staging downloads, verifies, and writes the release to the inactive A/B slot. **A staged release does not activate on reboot** — the slot flip is unfinished (OSDIST-FLIP-01: the bootloader entry is pinned to slot-a at install time and the initramfs never reads `boot-state.json`), so the box keeps running the current version until that lands. See [GETTING-STARTED.md → Upgrading](GETTING-STARTED.md#upgrading). |
