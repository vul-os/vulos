...
| **Native Pairing** | Shows this box's LAN TLS certificate fingerprint (SPKI SHA-256) and box name/address as a QR code and copyable text, for a native client to pin on first connection instead of trusting a public certificate authority — the same pairing payload `vulos -print-pairing` prints at the console. **This is preparation, not a working flow yet**: `clients/core/` implements the pin/verify/store logic, but no shell (desktop, Android, or iOS) calls it and there is no camera-scanning UI anywhere, so nothing today can actually scan this code and complete a pairing handshake. See [NETWORKING.md](NETWORKING.md#native-client-pairing-pinning-this-boxs-lan-certificate). |
