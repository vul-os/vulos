...
| **Storage Mode** | Where the wider bundle (mail, office, OS) reads and writes its objects: **Central Tigris** (default — every read/write goes straight to hosted Tigris) or **Local MinIO + CRDT sync** (a co-located MinIO becomes the source of truth and the sync layer replicates to peer instances). Changing this needs a restart of `vulos-bundle.target` to take effect. |
...
| **Control Plane** *(owner only)* | Shows which control plane this box authenticates against for account sign-in and instance routing, with a live reachability indicator. Self-hosters can point it at their own control-plane deployment instead of the default; changing it re-scopes your account link and device enrolment, so you'll likely need to re-link afterwards. |
| **Relay & Reachability** *(owner only)* | Picks the provider used to reach this box from outside your network and for box-to-box rendezvous: the default relay, your own STUN/TURN, a libp2p Circuit Relay v2 peer, a WireGuard mesh (Tailscale/Headscale/Nebula), or none at all if this box has a static IP. This only changes ingress/rendezvous — real-time call audio (Meet/Talk) keeps using its own ICE/TURN path regardless, so you can't accidentally break call quality while changing how the box itself is reached. A **Test** button TCP-probes whatever is currently configured. |
...
| **Plan & Billing** | Shows usage against your account's storage, backup, and relay entitlements. On a fully standalone box with no external control plane connected, most of this reads as "not configured" — it only becomes meaningful once you've linked one. |
