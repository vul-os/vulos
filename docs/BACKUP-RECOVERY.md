...
Not covered, and the manifest says so: Talk/Meet history, Board/Docs documents, per-app data, and content held only on a peer via a content-blind share (which the server cannot decrypt — that is the guarantee working, not a gap; export it from the instance holding the keys).
