...
### Mitigations in code
- **Sandbox render, no same-origin** — the primary in-shell render is a sandboxed iframe in an *opaque* origin (`allow-scripts`, deliberately **no** `allow-same-origin`); AI apps are not in `firstPartyIds`, so `needsSameOrigin()` can never grant them the OS origin (`src/shell/Window.tsx`, `src/core/AppRegistry.ts`). The Settings "Open" button no longer does `window.open(.../html)` top-level — it opens the app in the same opaque-origin sandboxed iframe (`AIAppPreview` in `src/core/Settings.tsx`).
