...
| D101 | 2026-08-17 | **Partially reverses D96-D**: a name-constrained private root, owner-installed, for the BROWSER surface only — no public CA can issue for `.local` (CA/B Forum Internal Names), so a padlock offline needs a root the owner installed. CA key never on the box; issuance is CSR-based so native SPKI pinning is untouched. Constraint enforcement measured on Go/OpenSSL/NSS/Security.framework; **Chrome, Firefox, Android and iOS NOT measured** | **built end to end, unmeasured on phones** — issuance + tool + box-side SNI/expiry fallback + root distribution (box stores the root, Settings → Browser Trust hands it out with its SHA-256 and per-OS steps). Still NOT performed on any Android or iOS device |
...
**C. Signing/integrity (SIGNING.md, SIGN-/VERITY-).** **dm-verity** on the squashfs + signature verification at each boot stage (shim→bootloader→kernel→initramfs→squashfs). **Offline** signing (air-gapped/HSM), **root-signs-intermediate** PKI (offline root signs a fetched release-key cert; images signed by the release key). Key rotation/revocation via a **single monotonic "minimum trusted epoch"** — devices store a floor that rises but never falls and refuse anything below it (also gives rollback/downgrade protection). *Implementation note (`8846d61c`): the floor is raised from the **root-signed release cert**, not from "the highest epoch seen". A release-key-signed manifest is checked against the floor but may never move it — otherwise the compromised release key this mechanism exists to retire could publish `min_epoch = MaxInt64` and permanently brick every device's update path. One authority (the offline root, via `issue-release-cert -min-epoch N`), not two.* Counter in **TPM** where available, **sealed file** otherwise. No CRL, no clock dependence. **All Go** (per language decision below).
...
**UPDATE (D101, 2026-08-17) — PARTIALLY SUPERSEDED, for browsers only.** Everything above stands for **native clients**: they still pin the SPKI, no CA is in their trust path, and D101 does not change a line of that. What this entry did not have was an answer for a **browser**, where pinning is not available as a flow a non-technical owner can complete (see E below, which made the same point about why a native client was needed at all). D101 adds a **name-constrained root the owner installs** for that surface specifically. The "removes roughly 150 root CAs" claim above needs one correction in light of it: the honest comparison is not *one root vs none*, it is *one root the owner controls and that is cryptographically limited to `.local`/`.lan`/`.home.arpa`/`lan.vulos.org` and private IP space* vs *~150 roots, each unconstrained, that are already on the device*. Native-client SPKI pinning is preserved by construction — issuance is CSR-based over the box's existing key, so re-issuance never moves the pin. See D101 for the measurement, and for the four verifiers that were NOT tested.
...
**UPDATE (D98, 2026-08-07) — the Android call above is RESOLVED, box-side.** `backend/internal/unifiedpush` + `backend/services/notify` now implement the UnifiedPush send-path: a user registers a distributor endpoint, the box POSTs to it alongside (never instead of) Web Push, through the identical owner-targeting/DND choke point and SSRF guard shape Web Push already uses. See D98 below for the full account, including its mutation-tested SSRF/auth gates. This closes the "whether to add UnifiedPush as an Android option" question this entry originally left open — the answer was yes, and it shipped. What's still open is narrower than before: no Settings UI exists yet to register an endpoint (the API is there; nothing calls it), so this is a capability, not yet a feature a non-technical user can reach. The iOS call directly above is UNCHANGED — D98 does not touch it, does not weaken it, and does not add any sovereign option for iOS; that platform's APNs requirement stands exactly as stated.
...
## D98 (2026-08-07) — UnifiedPush (UP-CELL-01) implemented box-side, alongside Web Push

D96-F left two open calls: whether to add UnifiedPush as an Android option, and how prominently to document the iOS APNs exception. This entry closes the first; the second is unchanged — the iOS framing in `docs/USER-GUIDE.md`, `docs/ARCHITECTURE.md`, and `docs/ASSISTANT.md` stays exactly as strict as D96-F left it, and this work does not touch it beyond a cross-reference.

**A. What shipped.** `backend/internal/unifiedpush` — a standalone package mirroring `backend/internal/webpush`'s shape (`Config`/`Endpoint`/`Store`/`Sender`/`LiveSender`) for the UnifiedPush spec's plain-endpoint contract: a user registers a URL their chosen distributor (e.g. ntfy) handed them, and the box POSTs a JSON notification payload to it directly — no vendor, no RFC 8291 envelope (UnifiedPush doesn't specify one; that's a Web-Push-specific mechanism). `backend/services/notify/unifiedpush_service.go` wires it into the SAME choke point Web Push already uses — `Service.SendNotification` calls `maybeUnifiedPush` right after `maybeWebPush`, both gated by the identical owner-targeting rule and the identical `SuppressFunc`/DND predicate, so a UnifiedPush endpoint cannot become a side door around the checks Web Push already honours. `cmd/server/routes_notify_unifiedpush.go` adds `POST`/`DELETE /api/notifications/unifiedpush/subscribe` and `GET /api/notifications/unifiedpush/status`, session-gated the same way the Web Push subscribe routes are (X-User-ID from the auth middleware; none of these are in `publicPaths`).

**B. SSRF.** A user-supplied endpoint URL is exactly the shape of an SSRF vector, so it gets the same two-layer defense Web Push subscriptions already have, reusing `backend/internal/safedial` (not a second deny-list): a registration-time host screen (`unifiedpush.Validate`) and a send-time resolved-IP re-screen inside a guarded `http.Client` (defeats DNS rebinding). Loopback, link-local, cloud metadata, and the box's own LAN (RFC1918/CGNAT) are all denied — `allowLAN` is hardcoded false, so even a self-hosted distributor on the box's own network is out of scope for this guard, matching the literal requirement that the box must not become a request generator against its own network.

**C. Prune policy.** Mirrors Web Push exactly: an endpoint the distributor reports gone (404/410, RFC 8030 §8.3's convention) is deleted from the store on the next send attempt. A merely-failing (5xx/timeout) endpoint is left alone — pruning on a transient error would silently drop a user's registration over a blip.

**D. Flag-gated and additive, verified by mutation.** `VULOS_PUSH_UNIFIEDPUSH_ENABLE` unset (the default) ⇒ `Config.Enabled` false ⇒ registration 503s and nothing is ever sent — byte-identical behavior to before this package existed. Both hard security requirements were mutation-tested: dropping the SSRF screen in `unifiedpush.Validate` failed `TestValidate_SSRF` and `TestUnifiedPushSubscribe_RejectsSSRFEndpoint`; dropping the `owner == ""` gate in the subscribe handler failed `TestUnifiedPushSubscribe_RequiresAuth`. Both mutations were reverted after confirming the failure.

**E. The honest gap — this is NOT "push is now fully sovereign."** Two things this does not give a user:

1. **No client UI yet.** This wave is backend-only (`backend/` and `docs/` per scope; `frontend/`/`clients/` were explicitly out of scope). A user cannot today click a Settings toggle to paste in a UnifiedPush endpoint — the API exists, but nothing calls it. Registering one requires a manual `POST` today. Wiring the Settings → Notifications UI to obtain an endpoint from an installed distributor and call this API is separate, unstarted work.
...
## D101 (2026-08-17) — A name-constrained private root for the BROWSER surface; D96-D's pinning stands for native clients

**This partially reverses [D96-D](#d96-2026-08-04--native-installable-clients-and-the-trust-model-for-a-local-box).** That entry chose SPKI pinning *precisely* to keep certificate authorities out of the trust path, and called removing "roughly 150 root CAs" a strength rather than a limitation. It was right about native clients and it is unchanged for them. It did not have an answer for a browser, and the founder has now asked for one.

**A. What the founder asked for, and the one thing that makes it hard.** Three properties at once on a box's LAN address: encrypted, works with the internet down, and a **green padlock in an unmodified browser** — including Chrome on Android — ideally on `vulos.local`. Two of those are free today. The padlock is not.

**No publicly trusted CA can ever issue for `vulos.local`.** This was verified against the primary source rather than assumed, because the whole design rests on it. The CA/Browser Forum's [Internal Names guidance](https://cabforum.org/working-groups/server/internal-names/) states that "the issuance of certificates with a reserved IP address or internal server name is prohibited" (effective 2015-11-11) and that "On October 1, 2016, all publicly trusted SSL/TLS certificates with an internal name or reserved IP address will be revoked and/or blocked by browser software". An *Internal Name* is defined there as one that "cannot be verified as globally unique within the public DNS … because it does not end with a Top Level Domain registered in IANA's Root Zone Database". `.local` is exactly that, and is separately reserved by RFC 6762. A bare RFC1918 address is a *Reserved IP Address* under the same rules.

So the padlock has exactly one route: **a CA the devices trust because the owner installed it.** That is the accepted cost — **one root certificate, installed once per device** — and it was put to the founder and accepted. Everything else in this design exists to make that root the smallest concession it can be.

**B. Why this is materially smaller than what a public CA buys you.** The instinct behind D96-D — "no CA in the path" — is sound, and the honest comparison is not "one root vs none", it is "one root vs the ~150 already in every device".

- **A public root is unconstrained by construction.** Any one of the ~150 can mint for any name on earth, and historically some have (DigiNotar, TrustWave, the CNNIC/MCS intermediate). A browser trusts all of them for everything, always.
- **This root is constrained by construction.** Its X.509 `permittedSubtrees` limit it to `local`, `lan`, `home.arpa`, `lan.vulos.org` and private/link-local IP space — none of which resolve on the public internet. **A stolen copy of this CA's private key cannot mint a working certificate for `google.com`**, on any verifier that enforces the extension.
- **The owner controls it.** They generate it, they hold it, they can delete it from a device in one step. No third party is trusted, and nothing is added to the ~150 that a *public* attacker can reach.
- **The key is not on the box.** Issuance runs from an operator tool (`backend/cmd/vulos-lanca`), never from `vulos-server`. A CA sitting on the machine it certifies is the weak form of this idea: whoever takes the box takes the authority. `lanca.CheckNotOnBox` refuses to write the key under `/var/lib/vulos`, `/etc/vulos` or `/run/vulos`.
- **The box's private key never moves either.** Issuance is CSR-based: the box signs a CSR with the key `loadOrCreateKey` already persists, and the CA signs over that same public key. The CA never sees, transports or stores box key material.

**C. Native clients are unaffected — and that was measured, not assumed.** D96-D's pinning promise was "pin the SPKI, not the certificate, so the box can renew without forcing every paired device to re-pair". Because issuance is CSR-based over the box's existing key, **every certificate this CA issues carries the same SubjectPublicKeyInfo that clients pinned at pairing**. `TestReissuanceDoesNotChangeTheBoxSPKIPin` proves it across a pairing plus two issuances with different names, different lifetimes and different serials; the mutation that makes issuance generate a fresh key turns it red with the two differing pins printed. Verified end-to-end against the built binary too: the pin `vulos-lanca` printed is byte-identical to the one `openssl` computes from a box key it never touched.

**D. Name-constraint enforcement — what was actually measured, and what was not.** A constraint that is silently ignored is *worse* than none, because it invites a false sense of safety. So this was measured against four independent X.509 implementations rather than assumed:

| Verifier | Enforces? | Evidence |
|---|---|---|
| Go `crypto/x509` | **yes** | `x509: … not permitted by excluded constraint` |
| OpenSSL 3.6.3 | **yes** | `error 47 at 0 depth lookup: permitted subtree violation` |
| NSS (`vfychain`) | **yes** | `Chain is bad!` / `ERROR -8157` |
| Apple Security.framework (`security verify-cert`, macOS) | **yes** | `CSSMERR_TP_INVALID_CERTIFICATE` |

Each test runs **two controls** before it measures anything: the legitimate `vulos.local` leaf under the constrained root must verify, *and* the identical `google.com` forgery under an otherwise-identical **unconstrained** root must verify. With both green, the only difference between accept and reject is `permittedSubtrees`. This matters concretely — NSS reports a name-constraint violation as `ERROR -8157: Certificate extension not found`, which reads like a completely different failure, so a test pinned to that string would have been pinning a coincidence.

**NOT measured, and not claimed:** **Chrome** (BoringSSL + Chrome Root Store), **Firefox** (mozilla::pkix — *not* the NSS verifier tested above), **Android** (Conscrypt), and **iOS**. Security.framework is the shared macOS/iOS trust stack, which makes it the most transferable of the four, but a macOS binary is not an iOS device. **Nothing here has been run on a real Android or iOS device**, and the Android/iOS install flows in [LAN-TRUSTED-CERT.md](LAN-TRUSTED-CERT.md) are written from vendor documentation, not from a device.

**E. The self-signed certificate does not go away.** It stays as the SNI-selected fallback, and a device without the root must always connect **with one click — never a hard failure, never a lockout**, including the very first connection, where the owner is downloading the root over a connection they have not yet trusted. Two consequences that are requirements, not preferences:

1. **An expired or unrenewable leaf must fall back to self-signed, not brick the box.** Renewal depends on an operator running a tool; a box offline for a year must stay reachable. Leaves are 397 days (one day under the 398-day ceiling Apple and Chrome apply to public chains; both document that locally-installed roots are exempt, but the cost of staying under the line is one renewal a year and the cost of being wrong is an owner locked out of their own box). Short-lived leaves are deliberately *not* used: that habit exists because expiry approximates revocation for public CAs, and this root has no OCSP responder or CRL a browser would fetch.
2. **The CA leaf covers a SUBSET of the box's names.** `internal/lan`'s `NameSet` includes bare single labels (`vulos`), and RFC 5280 `permittedSubtrees` cannot express "any single label". So SNI selection must serve the self-signed certificate for any name the CA leaf does not carry.

**F. The SAN list is DERIVED, never hardcoded.** `[]string{"vulos.local", lanHost}` — the literal that appeared in three places — is a guess that stops being true the moment avahi renames a colliding box to `vulos-2.local` or an owner sets `VULOS_HOSTNAME`. That disagreement *is* the two-box collision's mechanism, and it would reappear in CA-issued certificates unless the name set has one source of truth. It now does: `internal/lan/names.go`. `lanca.NewNameSet` refuses an empty set rather than substituting a default, so a caller whose derivation failed gets an error instead of a plausible-looking wrong certificate.

**G. What would flip this back.** Any of:
- **A verifier that matters stops enforcing name constraints.** The entire "materially smaller than 150 public roots" argument rests on enforcement. If Chrome or Android is measured and does *not* enforce, this root is just an unconstrained CA on the owner's devices and the trade collapses.
- **`.lan` is delegated as a real TLD.** It is currently a de-facto, undelegated private TLD, not a reserved one. Delegation would let a stolen key mint for a publicly resolvable name; that entry must be removed if it happens.
- **A browser ships a workable alternative** — a first-class local-network HTTPS story, or general availability of something like the CA/Browser Forum's periodically-revisited short-lived-local-certificate proposals — that reaches a padlock without a user-installed root.
- **The root install flow proves too hard in practice.** If owners cannot complete it (Android's standing "network may be monitored" notice and iOS's second trust step under About → Certificate Trust Settings are both real friction), the honest answer is to keep the one-click warning and put the effort into the native client instead.

**H. Box-side integration — landed 2026-08-17, after this entry was first written.** The original H said the SNI selection, the expired-leaf fallback and the root-distribution surface were "not implemented yet" because `internal/lan` was held by a concurrent agent. All three have since landed and that sentence is now wrong; it is corrected rather than deleted so the sequence stays legible.

- **SNI selection and expiry fallback** are in `internal/lan/fileloader.go`. The on-disk leaf wins only when it is currently valid *and* covers the requested `ServerName`; otherwise the self-signed source serves. Deliberately not attempted: serving the CA leaf only to devices that have the root — **the handshake carries no such signal**, browsers do not send `certificate_authorities` in a ClientHello, and it is not needed, because a device without the root sees the same one click it would have seen anyway.
- **The box now holds the root** at `/var/lib/vulos/tls/lan-root.crt` (`internal/lan/lanroot.go`). This was a real gap in the design as written, not merely an unbuilt surface: issuance happens off-box, so the box held its leaf and nothing else, and the one machine every device on the LAN can reach had no copy of the one file a human has to move. The puller (`lancert_puller.go`) carries it in an optional `root_pem` field — optional so an issuer predating this keeps working — and a rejected root never fails the leaf install.
- **Two shapes are refused, on write and again on read.** A non-CA (a leaf copied to the root path installs cleanly into a trust store and validates nothing), and an **unconstrained** CA. The second is the property B rests on: a root with no `permittedSubtrees` vouches for any name on earth on every device it lands on, and no OS install dialog shows the difference. `VULOS_LANCERT_ALLOW_UNCONSTRAINED_ROOT` overrides it knowingly. Re-checking on READ is not belt-and-braces — the documented manual flow copies a file straight to the path and never passes the puller. A root that did not issue *this box's* leaf is refused too: it yields no padlock and a standing authority, which has no benign version.
- **`Settings → Network → Browser Trust`** (`frontend/src/core/settings/LANRootCertPanel.tsx`) is the owner-facing surface: SHA-256 fingerprint, download, QR to an absolute URL built from the box's **LAN IP** (a phone that cannot resolve `.local` is the case the flow exists for), what the root is limited to read off the certificate, and per-OS install steps. It is a **separate panel from Native Pairing**, adjacent to it: the two answer the same owner question with opposite trust models, and one panel showing a "fingerprint" that means a box public key in one half and a CA digest in the other would invite comparing the wrong two values.
- **The first fetch is not authenticated and the panel says so.** The owner downloads the root over TLS the device does not yet trust — that is the situation by definition and the box cannot engineer it away. What makes it acceptable is that the fingerprint is verifiable out of band, so the fingerprint is rendered *above* the download button (a test asserts the document order) with the two commands that print the same value. Both routes are session-gated like `/api/lan/pairing`; the cost is that a phone signs in before following the QR, which the panel states rather than leaving it at a raw 401.

**Still not measured, and still not claimed:** every per-OS instruction, in the panel and in [LAN-TRUSTED-CERT.md](LAN-TRUSTED-CERT.md), is transcribed from vendor documentation. **Nothing has been performed on a real Android or iOS device by anyone on this project**, and Chrome, Firefox, Android and iOS remain absent from the enforcement table in D above. The panel labels each platform guide individually rather than carrying one global disclaimer, so no platform can inherit a neighbour's credibility.
