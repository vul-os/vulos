...
| D101 | 2026-08-17 | **Partially reverses D96-D**: a name-constrained private root, owner-installed, for the BROWSER surface only — no public CA can issue for `.local` (CA/B Forum Internal Names), so a padlock offline needs a root the owner installed. CA key never on the box; issuance is CSR-based so native SPKI pinning is untouched. Constraint enforcement measured on Go/OpenSSL/NSS/Security.framework; **Chrome, Firefox, Android and iOS NOT measured** | **issuance + tool landed; box-side SNI/fallback NOT implemented** |
...
**H. What is NOT decided here, and is not built.** The SNI selection in `certsource.go`, the expired-leaf fallback in `fileloader.go`, and the box-side CSR/root-distribution surface are **not implemented yet** — `internal/lan` was held by a concurrent agent during this work. The issuance path, the constraints and the operator tool are done and tested; the box-side integration is not. Do not read this entry as "shipped".
