...
**Multi-instance sync (narrower than originally designed).** The design goal is a leaderless CRDT where every instance holds a full mergeable copy and concurrent writes converge without a leader. **That whole-database merge does not run today** — cr-sqlite is not integrated. The mechanism that replaces it is now built but **not wired into anything**: pure-Go change capture over SQLite's compiled-in SESSION extension (`backend/internal/sqlcrdt`) feeding a delta-state CRDT engine (`backend/internal/crdtsync`), neither of which is imported by any server code (details in the Multi-instance sync section below). What is real today: a leaderless CRDT for the **app registry only** (`backend/internal/multiinstance/`), synced same-LAN via mDNS (`backend/internal/fabric/`) and gated behind `VULOS_LAN_ENABLE=1`; and a real S3 cold-path **snapshot of one instance's local DB file** (`backend/services/sync/`), not a cross-node merge. See roadmap/SYNC.md and roadmap/CLUSTER.md for the full reality check and the forward plan (a shared DMTAP-substrate Sync spec, relay as WAN rendezvous).
...
- **Hot path (not wired)**: the intent is for live instances to stream changesets directly over the peering mesh (relay fallback for NAT/cross-location). An earlier transport implementation streamed `crsql_changes`, but that table never populated — cr-sqlite is not integrated — and the dead code was removed. **No data moves over this path today.**

  The mechanism is, however, no longer an open question. The blocker was thought to be the no-CGO rule (D23 / D94-J), which prevents loading cr-sqlite. That rule stands; the *inference* drawn from it did not. `load_extension` being unauthorised under `modernc.org/sqlite` blocks **third-party** loadable extensions only — it says nothing about upstream extensions compiled into the amalgamation, and SQLite's own **SESSION** extension is compiled in. Two pure-Go packages now exist:

  - `backend/internal/sqlcrdt` — **change capture**. Diffs the synced tables against an attached baseline database via `sqlite3session_diff`, so writes from any connection or package are captured with no change to a single existing write path. Verified end to end (session created, changeset applied to a second DB, a real data conflict driving the conflict callback) under `CGO_ENABLED=0` on darwin/arm64, linux/amd64 and linux/arm64, and that probe is kept as a permanent regression test.
  ...
  **Neither package is imported by anything outside itself.** Capture is capture and merge is merge — no server code constructs either, `crdtsync.RegisterHandlers` has no caller, and no data crosses instances as a result. Read this as "the mechanism is chosen and built in isolation", not as "whole-database sync works".
