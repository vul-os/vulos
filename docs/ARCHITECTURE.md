...
flowchart TD
    ...
    subgraph Backend["Go HTTP backend (backend/cmd/server/)"]
        ...
        Peering["Peering / fabric (Ed25519, CRDT sync, VulosID)"]
...
**Multi-instance sync (real, for four domains over the LAN).** The design goal is a leaderless CRDT where every instance holds a full mergeable copy and concurrent writes converge without a leader. cr-sqlite is not integrated and never will be; the replacement is pure-Go change capture over SQLite's compiled-in SESSION extension (`backend/internal/sqlcrdt`) feeding a delta-state CRDT engine (`backend/internal/crdtsync`), and it is now **wired and running** — `cmd/server/main.go` calls `startCRDTSync` with the LAN-only mux, and the account, settings, reminders and the security audit log replicate between boxes. Two limits define it: it runs **only where the LAN layer runs** (`VULOS_LAN_ENABLE=1`), and **which data syncs is an enforced allow-list** — today `users`, `profiles`, `reminders` and the audit log, with every refusal recorded beside its reason. Details in the Multi-instance sync section below. Alongside it: a leaderless CRDT for the **app registry only** (`backend/internal/multiinstance/`), synced same-LAN via mDNS (`backend/internal/fabric/`) and gated behind `VULOS_LAN_ENABLE=1`; and a real S3 cold-path **snapshot of one instance's local DB file** (`backend/services/sync/`), not a cross-node merge. See roadmap/SYNC.md and roadmap/CLUSTER.md for the full reality check and the forward plan (a shared DMTAP-substrate Sync spec, relay as WAN rendezvous).
...
| `backend/services/peering/` | Ed25519 peering, VulosID key lifecycle, single reachability seam (`resolvePeerBaseURL`) over the relay, Drop |
...
- **Hot path (real, narrow)**: live instances exchange deltas directly over the LAN fabric. An earlier transport streamed `crsql_changes`, but that table never populated — cr-sqlite is not integrated — and that dead code was removed. What replaced it works.
  ...
  - `cmd/server/crdtsync_wiring.go` — the wiring. `POST /api/crdt/pull`, `POST /api/crdt/push`, `GET /api/crdt/status` and `GET /api/crdt/sync-status` are mounted on the **LAN-only `fabricMux`**, never the public surface, behind a per-peer Ed25519 signature from a **rostered, non-revoked** instance. The shared fabric secret is accepted only while the box has learned no peer identity at all — a bootstrap window that closes at the first enrolment and never reopens, because a revoked box holds that same fleet-wide secret and an admission credential nobody can be evicted from is not one. A nil authorizer registers *nothing* and logs loudly rather than serving openly.
  ...
  **Two boundaries, easy to overstate in either direction:**
  ...
  - **An allow-list, not "the database syncs".** Domains are approved one at a time and refusals are recorded with reasons (`internal/crdtsync/policy.go`). Approved today: **`users`** (the account — the password hash replicates, or the account exists on your second box and cannot be logged into), **`profiles`** (settings), **`reminders`**, and the **security audit log** as a GROW-ONLY set (entries can be added everywhere and edited or deleted nowhere, so a compromised box cannot erase the record of its own compromise from the others). Refused on the record: `sessions` (a bearer token — one compromised instance would hand over live sessions everywhere), the key blobs and local API keys (usable directly rather than requiring a crack), per-device push endpoints, and node-local hardware facts like `storagemode` and `cgroup_slices` (copying one box's disk layout onto another describes hardware that is not there). `profiles` was refused while its row was a single JSON blob mixing `AIAPIKey` and `PinHash` with `Theme` and `Locale`; those credentials now live in a separate `profile_secrets` table that never replicates, which is what made the settings half safe to sync.
