...
6. **Apps** — two opt-out checkboxes, both pre-checked. Files, Calendar, and Contacts are always included.
   - *"Claim your Vulos username — enables Mail"* works: unchecking it persists `email: false`, and the Mail tile is the one entry in the launcher's suite-bundle map, so the tile disappears.
   - *"Install the productivity app — Diwan"* **currently does nothing.** The choice is persisted (`POST /api/setup/apps`, `workspace: false`), but nothing reads it: the launcher's `suiteBundleOf` map (`frontend/src/core/AppRegistry.ts`) contains only the Mail entry, so the `workspace` branch is unreachable, and no backend code consults the flag either. Diwan is a separate service with its own systemd unit rather than a launcher tile (see [APPS.md](APPS.md)), so there is no tile for the checkbox to hide and unchecking it does not stop the service being installed. Calendar/Contacts and mail connect to a mailbox you already own — there is no Vulos-hosted mailbox and no Vulos-hosted email address.
