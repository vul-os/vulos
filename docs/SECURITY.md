...
| `VULOS_PEER_ALLOW_LAN=1` | SSRF guard admits **every** private range to peer dials at once — RFC1918, CGNAT `100.64/10`, ULA, 6to4, NAT64 | All of them blocked |
| `VULOS_PEER_ALLOW_CIDR=<list>` | The same opt-in, narrowed to named CIDR blocks (`100.64.0.0/10` for a tailnet). Refuses any entry overlapping a bogon, loopback, link-local or multicast range, and refuses to start on malformed input. Unions with `VULOS_PEER_ALLOW_LAN`, which is the broader of the two | No private range dialable |
...
- **Peer shares / cross-instance fetches** go through a safe dialer that blocks private and LAN address ranges by default. `VULOS_PEER_ALLOW_CIDR=<blocks>` is the explicit, auditable opt-in — it names the ranges, so opening a `100.64.0.0/10` tailnet does not also open the home LAN. `VULOS_PEER_ALLOW_LAN=1` is the older coarse form that opens all of them at once. Under either, loopback, link-local (`169.254.169.254`), multicast and bogon ranges stay refused, and the effective grant is printed at every boot.
...
6. **Audit the opt-ins.** `VULOS_ASSISTANT_ALLOW_EXTERNAL`, `VULOS_SANDBOX_ENABLED`, `VULOS_PEER_ALLOW_LAN`, and every `*_ALLOW_INSECURE` variable should be unset unless you can say why not. If you need private-range peers, prefer `VULOS_PEER_ALLOW_CIDR` over `VULOS_PEER_ALLOW_LAN=1`: the boot log prints the grant either way, and one of the two is a list you can check against what you meant.
