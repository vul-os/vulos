...
func restoreGuide(cfg storage.Config) string {
	...
	if cfg.BYO {
		...
		b.WriteString("```\n\n")
		b.WriteString("Your data already lives there. Set the matching `S3_*` env vars in\n")
		...
		b.WriteString("Your data is currently in a managed bucket named `" + cfg.Bucket + "`.\n")
		b.WriteString("Copy it into your own bucket using the presigned URLs in\n")
		b.WriteString("`storage/objects.json`, or any S3 client. Then set the `S3_*` env\n")
	...
	b.WriteString("compatible editor (LibreOffice, Microsoft Office, the OSS Ofisi apps).\n")
	return b.String()
}
...
func selfHostCompose(cfg storage.Config) string {
	...
	b.WriteString("#\n")
	if cfg.BYO {
		b.WriteString("# Pre-filled for your BYO bucket. Provide S3_ACCESS_KEY_ID and\n")
		b.WriteString("# S3_SECRET_ACCESS_KEY in .env (not included in this export for security).\n")
	} else {
	...
	if cfg.BYO {
		b.WriteString(fmt.Sprintf("      S3_ENDPOINT: %q\n", endpoint))
		b.WriteString(fmt.Sprintf("      S3_REGION: %q\n", region))
		b.WriteString(fmt.Sprintf("      VULOS_UNIFIED_BUCKET: %q\n", bucket))
	} else {
		b.WriteString("      S3_ENDPOINT: \"http://minio:9000\"\n")
		b.WriteString(fmt.Sprintf("      S3_REGION: %q\n", region))
