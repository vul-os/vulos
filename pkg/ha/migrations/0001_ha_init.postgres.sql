-- 0001_ha_init.postgres.sql
-- Control-plane HA leader election state (RISK-SPOF-01) — Postgres dialect.
-- Uses BIGSERIAL/BIGINT instead of SQLite's INTEGER/AUTOINCREMENT.
...
CREATE TABLE IF NOT EXISTS ha_leases (
  ...
  generation     BIGINT NOT NULL DEFAULT 1     -- monotonic, bumps on every handoff
);
...
CREATE TABLE IF NOT EXISTS ha_peers (
  ...
  generation_seen  BIGINT NOT NULL DEFAULT 0   -- highest lease generation this peer has observed
);
