...
systemctl daemon-reload"
  ...
  # LANNAME-01, deploy path. The image path installs this at build time; a
  # deployed box installs dhcpcd5 and avahi-daemon from the SAME package list
  # and had the SAME defect, so it gets the same one file and the same two
  # hooks. See scripts/vulos-lan-ifaces.sh. Copied on every deploy so a fix to
  # the glob list reaches an existing box.
  scp -q "$(dirname "$0")/scripts/vulos-lan-ifaces.sh" "$DEPLOY_HOST:/usr/local/bin/vulos-lan-ifaces"
  ssh "$DEPLOY_HOST" sh -s << 'LANIF_EOF'
set -e
chmod 0755 /usr/local/bin/vulos-lan-ifaces
mkdir -p /etc/systemd/system/avahi-daemon.service.d
cat > /etc/systemd/system/avahi-daemon.service.d/vulos-lan-only.conf << 'DROPIN'
[Service]
ExecStartPre=/usr/local/bin/vulos-lan-ifaces --avahi
DROPIN
/usr/local/bin/vulos-lan-ifaces --dhcpcd
systemctl daemon-reload
systemctl try-restart avahi-daemon.service 2>/dev/null || true
LANIF_EOF
  echo "  ${GREEN}✓${NC} LAN identity: dhcpcd off app veths, avahi restricted to LAN interfaces"
...
# The kiosk launcher lives at scripts/vulos-kiosk.sh rather than in a heredoc
# here, for the same reason the config generator does: a heredoc can only be
# grepped, never run. As a file, TestKiosk* read the exact bytes the image gets
# and scripts/smoke-multiscreen.sh can execute it against a faked sysfs root.
# ── LANNAME-01: the box's own name must resolve to the box ───────────────────
#
# Measured on a booted arm64 box: `avahi-resolve -n vulos.local` answered
# 169.254.23.36 — an IPv4LL address dhcpcd had put on `vh_bae456`, one
# APPLICATION's veth — while the box itself was 10.0.2.15. The image installs
# dhcpcd5 and avahi-daemon and configured NEITHER, so both ran on Debian
# defaults: dhcpcd.service is "DHCP Client Daemon on all interfaces" and stock
# avahi publishes the host's name on every interface it can see.
#
# Both halves are configured from ONE file, scripts/vulos-lan-ifaces.sh, which
# owns the list of what an app link is. See its header for the measurements and
# scripts/smoke-lan-name.sh for the gate.
install -m 0755 "$(dirname "$0")/scripts/vulos-lan-ifaces.sh" \
  "$ROOTFS/usr/local/bin/vulos-lan-ifaces"

# dhcpcd's half is static, so it is applied here at build time. The stanza is
# GENERATED by the same script rather than written out again as a heredoc.
VULOS_DHCPCD_CONF="$ROOTFS/etc/dhcpcd.conf" \
  "$(dirname "$0")/scripts/vulos-lan-ifaces.sh" --dhcpcd

# avahi's half is NOT static: which interfaces are LAN interfaces is only
# knowable on the booted box, and app veths appear and vanish as apps start.
# ExecStartPre rather than a separate oneshot unit, so the allow-list is
# recomputed on every avahi start AND restart, not once at install time.
mkdir -p "$ROOTFS/etc/systemd/system/avahi-daemon.service.d"
cat > "$ROOTFS/etc/systemd/system/avahi-daemon.service.d/vulos-lan-only.conf" << 'EOF'
[Service]
ExecStartPre=/usr/local/bin/vulos-lan-ifaces --avahi
EOF

install -m 0755 "$(dirname "$0")/scripts/vulos-kiosk.sh" \
  "$ROOTFS/usr/local/bin/vulos-kiosk"
chmod +x "$ROOTFS/usr/local/bin/vulos-kiosk"

# The multi-output config generator, installed from the repo rather than
# written here as a second heredoc. scripts/vulos-kiosk-genconfig.sh is the one
# source: the image runs it and TestKioskGenConfig runs the same file with fake
# outputs, so what is tested is what ships.
install -m 0755 "$(dirname "$0")/scripts/vulos-kiosk-genconfig.sh" \
  "$ROOTFS/usr/local/bin/vulos-kiosk-genconfig"
