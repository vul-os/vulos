...
// behalf of a tenant.  Covered ranges:
//
//   - Loopback (127.0.0.0/8, ::1)
//   - Link-local unicast/multicast (169.254.0.0/16, fe80::/10) — covers the
//     AWS/GCP/Azure metadata endpoint (169.254.169.254) and GCP 169.254.169.254.
...
// (no loopback/RFC1918/CGNAT/link-local/metadata/ULA/multicast). Other CP
// subsystems that make outbound POSTs on behalf of a tenant (e.g. cell push
// send-on-behalf to browser-vendor push endpoints) reuse this same guard so the
// SSRF policy lives in exactly one place. Returns ErrWebhookURLBlocked /
...
// to the first approved address. Reused by other CP subsystems that POST to a
// tenant-influenced outbound URL. It does NOT follow redirects by default — the
// caller should set CheckRedirect to refuse them so a vendor 30x cannot bounce
// the request to an internal target after the parse-time screen.
...
//
// This prevents DNS-rebinding attacks: even if a tenant registers a hostname
// that initially resolves to a public IP, a re-keyed DNS answer returning a
// private IP at delivery time is detected and blocked by this dialer.
