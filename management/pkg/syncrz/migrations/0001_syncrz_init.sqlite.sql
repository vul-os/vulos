...
-- delta bytes and blob bytes live in the managed store under the per-account prefix.
-- For BYO/local-MinIO accounts, the box encrypts before push, so even the
-- the managed store bytes are opaque ciphertext from the coordinator's perspective.
--
...
-- The actual delta bytes live in the managed store under the account prefix, addressed
...
CREATE TABLE IF NOT EXISTS syncrz_deltas (
    ...
    payload_bytes   INTEGER NOT NULL, -- size of the opaque payload in the managed store (audit only)
    payload_sha256  TEXT NOT NULL,    -- content hash of the OPAQUE payload (dedup key)
    object_key      TEXT NOT NULL,    -- s3 key under the account prefix
...
-- Content-addressed blob manifest index. The blob bytes live in the managed store
-- under the account prefix; this table records "hash X exists for account Y"
-- so /pull and /blob can quickly resolve a hash to a the managed store key without a
-- LIST.
