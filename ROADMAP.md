...
## Office Suite (merged from vulos-office, now `office/`)

The productivity surface of Vulos lives in-repo at `office/` — a self-contained subtree
(React 18 / Vite / Tailwind frontend, JSX only; Go backend as a **nested Go module**,
`module vulos-office`). It is a self-hosted, open-source (MIT) suite for Documents, Sheets,
Slides, PDF, plus team chat/calling (Spaces) and Calendar/Meet. The OS shell consumes it via
the `@vulos/office-client` library and thin app wrappers under `apps/{office,spaces,calendar,meet}/`.

The throughline: the Office Suite never invents its own network. Collaboration, chat, and
calling reuse the **same Vulos peer fabric** the OS uses for device routing — P2P first
(cr-sqlite/CRDT sync to buckets, WebRTC for media), relay/TURN when direct connectivity fails.

> Storage backend, multi-location, co-location, identity/anchor-inbox, and the bundling/billing
> model for the Office Suite are governed by the OS-wide
> §"Storage Backend, Multi-Location, Co-location & Identity" section above — Office consumes the
> same unified S3 interface and `@vulos.org` identity; it does not select or provision its own
> backend. Office is bundled from the Suite (Starter) tier and up; there is no standalone Office tier.

The pillars below are **priority-ordered**.

### 1. Office Core — Documents, Sheets, Slides, PDF

A clean, single-binary suite that opens in the browser with no account required. Documents
(TipTap: rich text, tables, task lists, images, links), Sheets (Fortune Sheet: formulas,
multi-sheet, formatting), Slides (Reveal.js), and PDFs (annotate-and-sign canvas). Files
autosave to local JSON by default (PostgreSQL for multi-user); everything exports to `.docx`,
`.xlsx`, `.pptx`, `.pdf`, and Markdown.

- **Goals:** keep the local-first single-binary experience excellent and dependency-light;
  lossless import/export round-trips; crash-safe autosave; clean document models the
  collaboration layer can wrap in CRDTs without a rewrite.
- **Gaps to close:** fidelity edge cases (nested tables, merged cells, embedded media, slide
  transitions); local version history/snapshots; autosave robustness; PDF depth (page
  reorder/insert/delete/rotate, form-field detection, true text extraction).
- **Non-goals:** no macro/VBA/pixel-exact legacy layout; no telemetry/required account for the
  local single-binary mode; open inspectable JSON document models only.

### 2. Real-time Collaboration — Editing Over the Peer Fabric

Open the same Document, Sheet, or Slide deck on two Vulos instances and edit it live. Office
does not stand up a collaboration server — it rides the Vulos peer fabric (cr-sqlite/CRDT to
buckets, P2P with relay/TURN fallback). See the Vulos peering / RELAY layer.

- **Concrete features:** CRDT document model (text/sequence CRDT for Docs, grid CRDT for Sheets,
  tree CRDT for Slides) serialized to the same bucket/cr-sqlite sync the OS uses; fabric session
  join keyed by file ID; presence roster keyed to Vulos account; live cursors & selections;
  anchored threaded comments; suggestion (track-changes) mode; activity & version history from
  the CRDT op log.
- **Non-goals:** no bespoke collaboration backend; CRDTs only (no OT); no global hosted document
  cloud in the OSS core; no live co-editing inside the PDF annotator in v1.

### 3. PDF Auto-Sign — E-Signature with Cryptographic Audit Trail

A DocuSign-style signing flow on the existing PDF canvas: place signature/initial/date/text
fields, send a scoped expiring signing link per signer, and seal the result with a cryptographic
token + tamper-evident audit trail (who signed, when, from where, document hash before/after) and
a completion certificate.

- **Concrete features:** field-placement editor; per-signer scoped signing links; draw/type/upload
  signature; multi-signer & signing order (sequential/parallel); cryptographic token per signature
  bound to document hash; immutable append-only audit log; completion certificate + machine-readable
  manifest; verification checker; identity binding (prefer authenticated Vulos account, fall back
  to link + typed identity).
- **Non-goals:** not a hosted SaaS signing service; no eIDAS-QES/smartcard/HSM in v1; no paid
  KYC vendors in the OSS core; signing works self-hosted (cloud only relays the link/notification).

### 4. Vulos Spaces — Team Chat, Calling, and Meetings

A Slack + Google-Meet equivalent on the peer fabric: team channels, DMs, threads, presence, and
real-time voice/video calling, screen-share, and scheduled meeting rooms. Messaging syncs as CRDTs
over the same bucket transport; calling uses WebRTC P2P with Vulos relay/TURN fallback.

- **Concrete features:** public/private channels (CRDT-synced); DMs & group DMs; threads with
  unread/mention tracking; presence & status; 1:1 and group voice/video over WebRTC with relay/TURN
  fallback (TURN creds minted by the cloud signaling layer); screen-share; named/scheduled meeting
  rooms with join link + lobby; in-call essentials (mute/camera/active-speaker/roster/chat);
  signaling reuses the OS RELAY service.
- **Non-goals:** no third-party media stack (Zoom/Twilio/Agora); no giant-webinar scale in v1;
  no separate identity system (Spaces identity is the Vulos account); no bespoke signaling server;
  no PSTN dial-in in the OSS core.

### Office Suite — Future work

- **Multi-target builds:** each app surface (docs, sheets, slides, spaces, calendar, meet) builds
  as two targets — a standalone web build per subdomain (`office.vulos.org`, `calendar.vulos.org`,
  `meet.vulos.org`) and an embeddable library (`lib.jsx` export) consumed by the OS shell. Vite
  multi-entry config (`office/vite.config.*.js`) wires both outputs from the same source tree.
- **Deep-link routing per app:** canonical deep-link scheme (`vulos-office://docs/{id}`,
  `vulos-office://meet/{roomId}`, etc.) so the OS launcher, notification taps, and inter-app links
  navigate directly to a document, sheet, meeting room, or calendar event.
