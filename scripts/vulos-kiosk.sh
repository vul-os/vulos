...
# Create whatever XDG_RUNTIME_DIR resolved to, rather than a literal
# /run/user/0. A real boot sets nothing, so this is still `mkdir -p /run/user/0`
# there and the booted behaviour is byte-for-byte what it was.
#
# It was a literal path ABOVE the export, which meant the script was
# unrunnable — under `set -eu` it died right here — on any host that cannot
...
if [ "$screen_count" -gt 1 ] && command -v labwc >/dev/null 2>&1 && command -v cog >/dev/null 2>&1; then
  # Memory: WARN, NEVER REFUSE.
  #
  # roadmap/SCREENS-COST.md measured this box's shape: the first kiosk browser
  # costs ~322MB PSS, each one after it ~150-170MB, and three screens fits the
  # advertised 2GB floor with ~1GB still free. So there is no wall here to stop
  # anyone at — only a margin that thins. What this prints is information, and
  # the launch below runs identically whether it printed or not.
  #
  # Refusing would be the worse failure by a distance. A box that quietly drops
  # to one monitor because a launcher decided it knew better looks exactly like
  # broken hardware, on a machine whose only diagnostic surface is the monitors
  # it just declined to use — and this project's position is that the user's
  # hardware is the user's decision.
  #
  # VULOS_MEMINFO overrides where MemTotal is read from, exactly as
  # VULOS_DRM_ROOT and VULOS_DRI_ROOT do for connectors and render nodes, and
  # for the same reason: a macOS or container host has no /proc/meminfo worth
  # reading, so without the seam this decision could only be grepped.
  #
  # UNKNOWN memory must behave like an ordinary box, not a low one. A missing,
  ...
  # 3 GiB, i.e. "under roughly 3GB". MemTotal is memory the kernel can hand out,
  # always some way below what is on the board — a 4GB machine reports around
  # 3.7-3.9GB — so this threshold sits between a 2GB box (warns) and a 4GB box
  # (does not), which is exactly the line docs/GETTING-STARTED.md draws.
  if [ -n "$mem_kb" ] && [ "$mem_kb" -lt 3145728 ]; then
    echo "vulos-kiosk: warning: $screen_count screens on $((mem_kb / 1024))MB RAM — multi-screen is recommended at 4GB+ (see docs/GETTING-STARTED.md). Starting all $screen_count anyway." >&2
  fi
