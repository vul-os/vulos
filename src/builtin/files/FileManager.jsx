...
const SPREADSHEET_EXTS = new Set(['ods', 'xlsx', 'xls', 'csv', 'tsv'])
const DOC_EXTS = new Set(['md', 'txt', 'log', 'rtf', 'odt', 'doc', 'docx'])
...
function FileIcon({ name, isDir, isLink, className = '' }) {
  ...
  if (SPREADSHEET_EXTS.has(ext)) return <IconDocument className={`text-emerald-400 ${className}`} />
  if (DOC_EXTS.has(ext)) return <IconDocument className={`text-blue-400 ${className}`} />
