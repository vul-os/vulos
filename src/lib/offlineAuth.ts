// offlineAuth.ts — the OS offline auth gate (OFFLINE-AUTH-01).
//
...
// master-key crypto (lib/masterKey.ts): at online login the browser already
...
// failure modes without string-matching the message.
...
// masterKey.ts). We refuse to cache or unwrap anything weaker than this floor so
...
// `slot` is untrusted (network-fetched at cache time, or read back out of
// storage at unlock time) — kept `unknown` here; it is passed on to
// unwrapMasterKeyWithPassword() as `unknown` too, which does its own equivalent
// narrowing before using any field, so this function never needs to assert a
// stronger shape than it actually checks.
function validEnvelope(slot: unknown): boolean {
  return isRecord(slot) &&
    slot.kdf === 'pbkdf2-sha256' &&
    (Number(slot.iter) | 0) >= MIN_PBKDF2_ITERS &&
...
function idbStore(): Store {
  ...
  function tx(mode: IDBTransactionMode, fn: (os: IDBObjectStore) => IDBRequest): Promise<unknown> {
  ...
  return {
    ...
    set: (k, v) => tx('readwrite', os => os.put(v, k)).then(() => undefined),
    del: (k) => tx('readwrite', os => os.delete(k)).then(() => undefined),
  }
}
...
export function __setStoreForTests(s: Store): void {
  _store = s
}
...
export async function cacheEnvelope(slot: unknown): Promise<void> {
...
// greet them by name. Non-secret; never includes credentials. `identity` is
// caller-supplied (the login response's user object) — kept loosely typed
// (fields `unknown`) since this function never validated their type either,
// only their truthiness (`|| ''`), matching the original behaviour exactly.
export async function cacheIdentity(identity: { id?: unknown; name?: unknown } | null | undefined): Promise<void> {
  if (!identity) return
  await store().set(K_IDENTITY, { id: identity.id || '', name: identity.name || '' })
}

export async function getCachedIdentity(): Promise<unknown> {
...
let _unlockChain: Promise<unknown> = Promise.resolve()

export function unlockOffline(password: string): Promise<boolean> {
...
async function _unlockOfflineImpl(password: string): Promise<boolean> {
  const slot = await store().get(K_ENVELOPE)
  if (!slot) {
    throw taggedError('No offline access is set up on this device. Sign in online first.', 'NOT_ENROLLED')
  }
  if (!validEnvelope(slot)) {
    ...
    await wipe()
    throw taggedError('Your offline data is corrupted. Sign in online to restore it.', 'CORRUPT')
  }
  ...
  try {
    ...
    const attempts = (Number(await store().get(K_ATTEMPTS)) | 0) + 1
    await store().set(K_ATTEMPTS, attempts)
    if (attempts >= MAX_OFFLINE_ATTEMPTS) {
      await wipe()
      throw taggedError(
        'Too many attempts — offline data on this device has been wiped. Sign in online to restore it.',
        'WIPED',
      )
    }
    const left = MAX_OFFLINE_ATTEMPTS - attempts
    const e = taggedError(
      `Incorrect password — ${left} attempt${left === 1 ? '' : 's'} left before offline data is wiped.`,
      'WRONG_PASSWORD',
    )
    throw Object.assign(e, { attemptsLeft: left })
  }
...
export async function appKey(appId: string): Promise<CryptoKey> {
  const mk = getMasterKey()
  if (!mk) {
    throw taggedError('offlineAuth: locked — cannot derive an app key', 'LOCKED')
  }
