...
// routes_identity_claim_test.go — IDENTITY-01 coverage for the account-username claim
...
func TestIdentityCheck_ForwardsAndRelays(t *testing.T) {
	cp := newFakeCP(t, http.StatusOK, `{"address":"alice@example.com","available":true}`)
	t.Setenv("VULOS_CLOUD_API_URL", cp.srv.URL)

	mux := http.NewServeMux()
	registerIdentityClaimRoutes(mux)

	r := httptest.NewRequest(http.MethodGet, "/api/identity/check?handle=alice&domain=example.com", nil)
	...
	if !strings.Contains(cp.lastQuery, "handle=alice") || !strings.Contains(cp.lastQuery, "domain=example.com") {
	...
	if got["available"] != true || got["address"] != "alice@example.com" {
...
func TestIdentityClaim_ForwardsCookieAndRelaysStatus(t *testing.T) {
	...
	body := `{"handle":"alice","domain":"example.com","account_id":"attacker-supplied"}`
	...
	if forwarded["handle"] != "alice" || forwarded["domain"] != "example.com" {
...
func TestIdentityClaim_MissingHandle400(t *testing.T) {
	...
	r := httptest.NewRequest(http.MethodPost, "/api/identity/claim", strings.NewReader(`{"domain":"example.com"}`))
	r.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	mux.ServeHTTP(w, r)
...
func TestIdentityClaim_RequiresLocalSession(t *testing.T) {
	cp := newFakeCP(t, http.StatusOK, `{"address":"alice@example.com","claimed":true}`)
	t.Setenv("VULOS_CLOUD_API_URL", cp.srv.URL)
	...
	res, err := http.Post(srv.URL+"/api/identity/claim", "application/json",
		strings.NewReader(`{"handle":"alice","domain":"example.com"}`))
	if err != nil {
...
func TestIdentityClaim_NoDeviceHeadersWhenNotEnrolled(t *testing.T) {
	cp := newFakeCP(t, http.StatusOK, `{"address":"bob@example.com","claimed":true}`)
	t.Setenv("VULOS_CLOUD_API_URL", cp.srv.URL)
	...
	r := httptest.NewRequest(http.MethodPost, "/api/identity/claim",
		strings.NewReader(`{"handle":"bob","domain":"example.com"}`))
	r.Header.Set("Content-Type", "application/json")
