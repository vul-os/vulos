...
// process spawning.
//
// They assert against appCommandArgv and appProcessEnv in launcher_privilege.go
// — the functions launchWithConcurrency actually calls. That is load-bearing.
// Until this file was corrected, every assertion below ran against test-local
// copies named buildAppCommand and buildAppEnv, each introduced as "mirrors the
// exact command construction in launchWithConcurrency". Removing setpriv and all
// four of its flags from the launcher left the whole file green, because nothing
// here read the launcher. A guard that mirrors its subject tests the mirror.
//
...
func TestCommand_UsesSetpriv(t *testing.T) {
	args := appCommandArgv("vulos_test", "node server.js")
	found := false
	for _, a := range args {
		if a == "setpriv" {
			found = true
...
func TestCommand_DropsToNobodyUID(t *testing.T) {
	...
	args := appCommandArgv("vulos_test", "python app.py")
	for _, a := range args {
		if a == wantFlag {
			return
		}
...
// allowlist — not from os.Environ().  The server's own environment holds API
// keys, tokens and relay credentials, and an app process is not trusted with
// them.  Structural: starting appProcessEnv from os.Environ() displaces the
// hardcoded PATH from position 0, which is what this reads.
func TestEnv_NoOsEnviron(t *testing.T) {
	env := appProcessEnv(nil, 9000)
