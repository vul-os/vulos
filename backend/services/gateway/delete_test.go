...
func newDeleteGateway(t *testing.T) (*Gateway, string /*token*/, string /*userID*/, string /*localRoot*/) {
	t.Helper()
	...
	return g, token, userID, root
}

func doDelete(g *Gateway, token string, body any) *httptest.ResponseRecorder {
...
func TestDeleteHandler_Unauthorized(t *testing.T) {
	g, _, _, _ := newDeleteGateway(t)
	rec := doDelete(g, "not-a-real-token", deleteRequest{AppID: "office", Key: "a.txt"})
...
func TestDeleteHandler_AppNotPermitted(t *testing.T) {
	...
	rec := doDelete(g, token, deleteRequest{AppID: "notes", Key: "a.txt"})
...
func TestDeleteHandler_InvalidKeyRejected(t *testing.T) {
	...
	for _, k := range cases {
		rec := doDelete(g, token, deleteRequest{AppID: "office", Key: k})
...
func TestDeleteHandler_HappyPath_RemovesObject(t *testing.T) {
	...
	rec := doDelete(g, token, deleteRequest{AppID: "office", Key: "documents/report.docx"})
	if rec.Code != http.StatusNoContent {
		t.Fatalf("status = %d, want 204, body=%s", rec.Code, rec.Body.String())
	}
...
func TestDeleteHandler_MissingObjectIsNoop(t *testing.T) {
	g, token, _, _ := newDeleteGateway(t)
	rec := doDelete(g, token, deleteRequest{AppID: "office", Key: "never/existed.txt"})
	if rec.Code != http.StatusNoContent {
...
func TestDeleteHandler_CrossAppIsolation(t *testing.T) {
	...
	rec := doDelete(g, token, deleteRequest{AppID: "office", Key: "shared.txt"})
	if rec.Code != http.StatusNoContent {
		t.Fatalf("status = %d, want 204, body=%s", rec.Code, rec.Body.String())
	}
