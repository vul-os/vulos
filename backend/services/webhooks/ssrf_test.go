// ssrf_test.go — tests for the webhook SSRF guard (security-critical).
//
...
//
// Ported from management/pkg/webhooks/ssrf_test.go during the management ->
// vulos fold, adapted to the SQLite-file-per-package OpenStore/OpenStoreForTest
// API (no more cpdb dual-backend seam) — the SSRF assertions themselves are
// unchanged.
...
func openProdStore(t *testing.T) *webhooks.Store {
	t.Helper()
	st, err := webhooks.OpenStore("")
	if err != nil {
		t.Fatalf("webhooks.OpenStore: %v", err)
	}
...
func openBypassStore(t *testing.T) *webhooks.Store {
	...
	if err != nil {
		t.Fatalf("webhooks.OpenStoreForTest: %v", err)
	}
	t.Cleanup(func() { _ = st.Close() })
	return st
}
...
func TestCreateSubscriptionRejectsBlockedURLs(t *testing.T) {
	// Use the production OpenStore (with SSRF enabled), not OpenStoreForTest.
	st := openProdStore(t)
	...
	for _, tc := range blockedURLs {
		...
		t.Run(tc.name, func(t *testing.T) {
			_, err := st.CreateSubscription(ctx, "admin-ssrf", tc.url, []string{"device.enrolled"})
			if err == nil {
...
func TestCreateSubscriptionRejectsMetadataURL(t *testing.T) {
	...
	_, err := st.CreateSubscription(context.Background(), "admin-ssrf-meta",
		"http://169.254.169.254/latest/meta-data/iam/security-credentials/",
		[]string{"backup.completed"})
	if err == nil {
...
//
// The test uses OpenStoreForTest to insert a subscription pointing at a
// loopback httptest server, then dispatches via NewDispatcher (SSRF-safe).
// The test server must not receive any request.
func TestDeliveryRefusesPrivateIP(t *testing.T) {
	...
	// OpenStoreForTest bypasses URL validation so we can create a subscription
	// with a loopback URL, simulating what an attacker achieves via DNS
	// rebinding (the subscription-time check saw a public IP; dial time sees
	// private).
	st := openBypassStore(t)
	ctx := context.Background()

	_, err := st.CreateSubscription(ctx, "admin-rebind", srv.URL, []string{"device.enrolled"})
	if err != nil {
		t.Fatalf("OpenStoreForTest should bypass SSRF: %v", err)
	}
	...
	if err := d.Emit(ctx, "device.enrolled", json.RawMessage(`{"test":true}`)); err != nil {
		t.Fatalf("Emit: %v", err)
	}
...
func TestDeliveryRefusesRFC1918AfterRebind(t *testing.T) {
	...
	_, err := st.CreateSubscription(ctx, "admin-rfc1918", privateURL, []string{"device.removed"})
	if err != nil {
		t.Fatalf("OpenStoreForTest bypass failed: %v", err)
	}

	d := webhooks.NewDispatcher(st)
	if err := d.Emit(ctx, "device.removed", json.RawMessage(`{}`)); err != nil {
		t.Fatalf("Emit: %v", err)
	}
...
// validateBlockedHostTests exercises every category in isBlockedIP. It is
// internal to ssrf.go; we test the observable effect via CreateSubscription
...
func TestValidateWebhookURL_BlockedRanges(t *testing.T) {
	...
	for _, tc := range validateBlockedHostTests {
		...
		t.Run(tc.label, func(t *testing.T) {
			_, err := st.CreateSubscription(context.Background(), "admin-test", tc.url, []string{"device.enrolled"})
			if err == nil {
...
func TestValidateWebhookURL_PublicIPAllowed(t *testing.T) {
	...
	// We cannot insert a real subscription for a public IP because
	// CreateSubscription does an actual DNS lookup; for an IP literal it just
	// validates the IP range. Open a production store (SSRF enabled) and
	// check it does NOT error.
	st := openProdStore(t)

	_, createErr := st.CreateSubscription(context.Background(), "admin-public", publicURL, []string{"device.enrolled"})
	if createErr != nil {
...
func TestSsrfSafeDialer_BlocksLoopback(t *testing.T) {
	...
	_, err = st.CreateSubscription(ctx, "admin-dialblock", loopbackURL, []string{"backup.completed"})
	if err != nil {
		t.Fatalf("OpenStoreForTest bypass failed: %v", err)
	}
	...
	d := webhooks.NewDispatcher(st)
	_ = d.Emit(ctx, "backup.completed", json.RawMessage(`{"ok":true}`))
