...
// implementations for the Vulos box BYO-KMS feature.
//
// Ported verbatim (crypto unchanged) from management/pkg/kms/envelope.go.
...
// This is the key used to encrypt the owner's data. It is immediately wrapped
// by the owner's Provider; the box never persists the plaintext DEK.
...
//  2. Encrypts plaintext with the DEK via AES-256-GCM.
//  3. Wraps the DEK via the owner's Provider.
//
// Returns (ciphertext, wrappedDEK, error).
// ciphertext is hex(nonce||gcmCiphertext). wrappedDEK is the opaque blob
// returned by provider.WrapDEK; store it via Store.PutDEK.
...
//  1. Unwraps the DEK via the owner's Provider.
//  2. Decrypts ciphertext with the DEK via AES-256-GCM.
//
...
// ---------------------------------------------------------------------------
// SymmetricProvider — AES-256-GCM wrap/unwrap using owner's key material
// ---------------------------------------------------------------------------

// SymmetricProvider wraps/unwraps DEKs using an owner-supplied 32-byte AES
// key. The key is transmitted once at config time and stored on the box
// encrypted under the local KMS_STORAGE_KEK. This is the "local KMS" mode.
//
// Security note: the box briefly observes the owner key at configuration
// time and holds it at rest encrypted under KMS_STORAGE_KEK. For a stronger
// guarantee (the box never observes the KEK at all) use HTTPProvider.
type SymmetricProvider struct {
	key []byte // 32-byte AES key (owner KEK)
}
...
// WrapDEK encrypts plainDEK (32 bytes) with the owner's AES key.
// Returns hex(nonce||gcmCiphertext).
...
// ---------------------------------------------------------------------------
// HTTPProvider — delegates wrap/unwrap to owner-operated HTTP KMS endpoint
// ---------------------------------------------------------------------------

// HTTPProvider calls the owner's KMS endpoint (e.g. HashiCorp Vault transit,
// or a custom HTTPS service). The owner's KEK never leaves their
// infrastructure; the box only ever sees wrapped blobs.
//
...
//
// The owner's service must present a valid TLS certificate. Requests include
...
// e.g. "https://kms.owner.example/v1/transit".
...
// wrapped blob from the owner's KMS.
