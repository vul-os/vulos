...
type Session struct {
	...
	// fpsC signals the video pipeline goroutine to restart with a new FPS.
	// mangoHudC signals the pipeline to restart with MangoHud toggled.
	// Both use debounce channels (capacity 1, non-blocking send) so rapid
	// calls coalesce — only the latest value takes effect.
	fpsC      chan int
	mangoHudC chan bool
}
