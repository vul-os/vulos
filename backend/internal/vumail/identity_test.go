// identity_test.go — unit tests for VUMAIL-01 Provision (identity.go).
//
// Covers:
...
func TestValidateDomain(t *testing.T) {
	valid := []string{"vumail.org", "example.com", "mail.example.co.za"}
	for _, d := range valid {
...
func TestProvisionIdempotency(t *testing.T) {
	...
	dir := t.TempDir()
	store, err := NewStore(filepath.Join(dir, "vumail.db"))
	if err != nil {
	...
	pi, err := Provision(ctx, store, "acct-1", "alice", "vumail.org", "passphrase1", stub.Client())
	if err != nil {
		t.Fatalf("Provision (first call): %v", err)
	}
	if pi.Address != "alice@vumail.org" {
		t.Errorf("address = %q, want alice@vumail.org", pi.Address)
	}
	if callCount != 1 {
	...
	pi2, err := Provision(ctx, store, "acct-1", "alice", "vumail.org", "passphrase1", stub.Client())
	if err != nil {
		t.Fatalf("Provision (second call / idempotent): %v", err)
	}
	if pi2.Address != "alice@vumail.org" {
		t.Errorf("idempotent address = %q, want alice@vumail.org", pi2.Address)
	}
...
func TestProvisionHandleTaken(t *testing.T) {
	...
	_, err = Provision(context.Background(), store, "acct-2", "takenhandle", "vumail.org", "pass", stub.Client())
	if err == nil {
...
//
// The frozen invariant is: a vumail identity is mandatory.  This test asserts
...
func TestProvisionMandatoryIdentityInvariant(t *testing.T) {
	...
	_, err = Provision(ctx, store, "acct-3", "", "vumail.org", "pass", nil)
	if err == nil {
	...
	bypassAttempts := []struct {
		...
		{"  ", "vumail.org"},   // whitespace-only handle
		{"AB", "vumail.org"},   // too short + uppercase
		{"alice", ""},          // empty domain
