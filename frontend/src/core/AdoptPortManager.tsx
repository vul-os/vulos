...
function toAdoptedPort(x: unknown): AdoptedPort | null {
  ...
  return {
    ...
    requires_product: typeof x.requires_product === 'string' ? x.requires_product : undefined,
  }
}
...
async function apmList(): Promise<AdoptedPort[]> {
  const res = await fetch('/api/proxyadopt')
  if (!res.ok) throw new Error('list failed')
  const data: unknown = await res.json() // [{id, name, port, url, healthy, requires_product}]
  return Array.isArray(data) ? data.map(toAdoptedPort).filter((p): p is AdoptedPort => p !== null) : []
}

interface ApmAdoptArgs {
  name: string
  port: string
  requiresProduct?: string
}

async function apmAdopt({ name, port, requiresProduct }: ApmAdoptArgs): Promise<AdoptedPort | null> {
  const res = await fetch('/api/proxyadopt', {
    method: 'POST',
  ...
  if (!res.ok) throw new Error((isRecord(body) && typeof body.error === 'string' ? body.error : '') || 'adopt failed')
  return toAdoptedPort(body)
}

async function apmRevoke(id: string): Promise<unknown> {
...
interface ApmAdoptFormProps {
  onAdopted: () => void
}

function ApmAdoptForm({ onAdopted }: ApmAdoptFormProps) {
  ...
  const submit = useCallback(async (e: FormEvent) => {
    ...
    try {
      ...
      setErr(errorMessage(ex) || 'Could not adopt port.')
...
interface ApmRowProps {
  item: AdoptedPort
  onRevoke: () => void
}

function ApmRow({ item, onRevoke }: ApmRowProps) {
  const [busy, setBusy] = useState(false)
...
interface ApmPopoverProps {
  onClose: () => void
}

function ApmPopover({ onClose }: ApmPopoverProps) {
  const [items, setItems] = useState<AdoptedPort[] | null>(null)
