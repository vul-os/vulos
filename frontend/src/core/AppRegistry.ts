...
// The registry-entry shape shared by every source this module merges:
// hand-written builtins/default web apps (all fields literal, always present)
// plus backend-sourced installed/AI-generated apps (narrowed from `unknown`
// JSON in refreshInstalled/refreshAIApps below, so every field the network
// path fills stays optional here even where the literal arrays always set it).
export interface App {
  ...
  url?: string
  port?: number
  stream_browser?: boolean
  needsSameOrigin?: boolean
  command?: string
  workDir?: string
  installed?: boolean
  ai5AIApp?: boolean
...
const builtinRegistry: App[] = [
  ...
  //  2. "Chromium" (id: browser-stream) — a REAL Chromium instance
  ...
  {
    id: 'browser-stream',
    // Named for what actually runs. services/webbrowser/chrome.go resolves
    // chromium first and the image ships Chromium, not Chrome — calling it
    // "Chrome" in the UI was both inaccurate and someone else's trademark.
    name: 'Chromium',
    icon: 'chrome',
...
const suiteBundleOf = new Map<string, string>([
  ['lilmail', 'email'],
])
...
let suiteSelection: SuiteSelection = { email: true, workspace: true, chosen: false }
let suiteFetchPromise: Promise<SuiteSelection> | null = null

export function refreshSuiteSelection(): Promise<SuiteSelection> {
...
function suiteAppEnabled(id: string): boolean {
...
export function subscribeApps(fn: () => void): () => void { appListeners.add(fn); return () => { appListeners.delete(fn) } }
export function getAppsVersion(): number { return appsVersion }
function notifyApps(): void { appsVersion++; for (const fn of appListeners) fn() }
...
// Narrows one raw /api/store/installed record (untyped JSON from the network
// — a trust boundary) into an App, applying the same field-by-field fallbacks
// the original untyped JS used rather than trusting the shape.
function toInstalledApp(a: Record<string, unknown>): App {
  const id = typeof a.id === 'string' ? a.id : ''
  const name = typeof a.name === 'string' ? a.name : ''
  return {
    id,
    name,
    icon: (typeof a.icon === 'string' && a.icon) || name?.[0]?.toUpperCase() || '?',
    ...
    port: typeof a.port === 'number' ? a.port : 80,
    command: typeof a.command === 'string' ? a.command : '',
    workDir: typeof a.work_dir === 'string' ? a.work_dir : '',
...
export function refreshInstalled(): Promise<App[]> {
  fetchPromise = fetch('/api/store/installed')
    .then(r => r.ok ? r.json() : [])
    .then((apps: unknown) => {
      const raw = Array.isArray(apps) ? apps.filter(isRecord) : []
      installedApps = raw
        .filter(a => !builtinRegistry.some(b => b.id === (builtinAliases.get(String(a.id)) || a.id)))
        .map(toInstalledApp)
      notifyApps()
      return installedApps
    })
    .catch(() => { installedApps = []; return installedApps })
  return fetchPromise
}

// AI5: Dynamic AI-generated apps from ~/.vulos/ai-apps/
let ai5AIApps: App[] = []
let ai5FetchPromise: Promise<App[]> | null = null

// Narrows one raw /api/ai-apps record (untyped JSON from the network) into an
// App, same fallback shape as the original untyped JS.
function toAI5App(a: Record<string, unknown>): App {
  const id = typeof a.id === 'string' ? a.id : ''
  const title = typeof a.title === 'string' ? a.title : ''
  return {
    id,
    name: title || id,
    icon: typeof a.icon === 'string' && a.icon ? a.icon : '🤖',
    description: title || 'AI-generated app',
    keywords: ['ai', 'generated', ...(title ? title.toLowerCase().split(/\s+/) : [])],
    category: typeof a.category === 'string' ? a.category : 'ai',
    url: `/api/ai-apps/${id}/html`,
    ai5AIApp: true,
  }
}

export function refreshAIApps(): Promise<App[]> {
  ai5FetchPromise = fetch('/api/ai-apps')
    ...
    .catch(() => { ai5AIApps = []; return ai5AIApps })
...
export function getAppsByCategory(): Record<string, App[]> {
  const cats: Record<string, App[]> = {}
