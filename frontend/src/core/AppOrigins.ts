// AppOrigins.ts — ORIGIN-01: per-app browser origins.
//
...
// The subset of window.location that origin-minting needs. A plain object
// literal (as used in tests) satisfies this just as well as the real Location.
...
let originConfig: OriginConfig = { enabled: false, baseDomain: '', profile: DEFAULT_PROFILE }

// setOriginConfig is exported for tests and for the boot fetch below. `cfg`
// crosses a trust boundary (server JSON / a test literal), so it is narrowed
// explicitly rather than cast.
export function setOriginConfig(cfg: unknown): OriginConfig {
  const c = isRecord(cfg) ? cfg : {}
  const baseDomain = typeof c.base_domain === 'string' ? c.base_domain : ''
  originConfig = {
    enabled: !!(c.enabled && baseDomain),
    baseDomain,
    profile: typeof c.profile === 'string' ? c.profile : DEFAULT_PROFILE,
  }
  return originConfig
}

export async function loadOriginConfig(fetchImpl: typeof fetch = globalThis.fetch): Promise<OriginConfig> {
  try {
...
//
// `url` may be undefined (a window with no url yet) — `new URL()` requires a
// string, so it is stringified the same way the URL constructor itself would
// coerce a non-string argument (String(undefined) === 'undefined'), preserving
// the exact prior JS behaviour for that edge case.
export function originOf(url: string | undefined, base?: string): string | null {
  const href = base || (typeof window !== 'undefined' ? window.location.href : undefined)
  try {
    const o = new URL(String(url), href).origin
...
export function isDistinctOrigin(url: string | undefined): boolean {
...
export function iframeSandboxForURL(url: string | undefined): string {
...
export function expectedFrameOrigin(url: string | undefined): string {
  const o = originOf(url)
...
export function gatewayAppId(url: string | undefined): string | null {
...
export function appOriginURL(
  appId: string,
  cfg: OriginConfig = originConfig,
  loc: OriginLocation | null = typeof window !== 'undefined' ? window.location : null,
): string | null {
...
export function resolveAppFrameURL(url: string, cfg: OriginConfig = originConfig, loc?: OriginLocation): string {
...
export function appFrameURLFor(appId: string, cfg: OriginConfig = originConfig, loc?: OriginLocation): string {
