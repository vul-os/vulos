...
// CommandContext — assembled by the palette at dispatch time (see
// CommandPalette.jsx, out of scope/untyped). `run`/`when` only ever touch
// these named helpers (see the module doc above), so the shape is declared
// here rather than left as `any`.
 ...
 */
export function registerCommands(list: Command[] | null | undefined): () => void {
...
function safeWhen(when: (ctx: CommandContext) => boolean, ctx: CommandContext): boolean {
