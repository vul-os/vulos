...
// TelemetryStats — the fields this panel reads off useTelemetry()'s `unknown`
// live WS payload, narrowed here rather than trusted (mirrors the wire shape
// asserted by src/__tests__/BoxHealthPanel.test.tsx).
...
function toHealthStatus(x: unknown): HealthStatus | null {
  if (!isRecord(x)) return null
  const checks: Record<string, string> = {}
  if (isRecord(x.checks)) {
    for (const [k, v] of Object.entries(x.checks)) {
      if (typeof v === 'string') checks[k] = v
    }
  }
  return {
...
function humanMB(mb: number | null | undefined): string {
  if (mb == null) return '—'
...
function checkTone(v: unknown): string {
...
export default function BoxHealthPanel() {
  ...
  useEffect(() => {
    loadHealth()
    fetch('/api/system/info', { credentials: 'include' }).then(r => r.json()).then((data: unknown) => setSys(toSysInfo(data))).catch(() => {})
  ...
  return (
    <Section title="Box Health">
      ...
      <div className="space-y-px rounded-xl overflow-hidden border border-[var(--border-default)] mb-5">
        ...
        {(stats?.temp ?? 0) > 0 && <InfoRow label="Temperature" value={`${Math.round(stats?.temp ?? 0)}°C`} />}
