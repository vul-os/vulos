...
function toConflictItem(x: unknown): ConflictItem | null {
  if (!isRecord(x) || typeof x.path !== 'string') return null
  return {
    path: x.path,
    base_path: typeof x.base_path === 'string' ? x.base_path : '',
    node: typeof x.node === 'string' ? x.node : '',
    timestamp: typeof x.timestamp === 'string' ? x.timestamp : '',
    size: typeof x.size === 'number' ? x.size : 0,
  }
}

type Keep = 'ours' | 'theirs'
...
export default function ConflictResolver({ onClose }: ConflictResolverProps) {
  const [cl10Conflicts, setCl10Conflicts] = useState<ConflictItem[]>([])
  const [cl10Loading, setCl10Loading] = useState(true)
  const [cl10Busy, setCl10Busy] = useState<Record<string, Keep>>({})
  const [cl10Error, setCl10Error] = useState<string | null>(null)
  ...
  const cl10Fetch = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(data) ? data.map(toConflictItem).filter((c): c is ConflictItem => c !== null) : []
      setCl10Conflicts(list)
    } catch (err) {
      setCl10Error(errorMessage(err))
    } finally {
  ...
  const cl10Resolve = useCallback(async (path: string, keep: Keep) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const bodyMsg = isRecord(body) && typeof body.error === 'string' ? body.error : undefined
