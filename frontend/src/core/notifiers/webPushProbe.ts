...
import { pushSupported, type WebPushDeps } from './webPush'

function isRecord(x: unknown): x is Record<string, unknown> {
  return typeof x === 'object' && x !== null
}

export type PushProbeState = 'unsupported' | 'unconfigured' | 'denied' | 'on' | 'off'

export async function probeState(deps: WebPushDeps = {}): Promise<PushProbeState> {
  ...
  if (!isRecord(vapid) || !vapid.enabled || !vapid.publicKey) return 'unconfigured'
