...
export function setPendingLaunchQuery(appId: string | null | undefined, query: string | null | undefined): void {
  if (appId && query) pending.set(appId, String(query))
}

/** Read and clear the pending launch query for an app id (call once, on mount). */
export function consumePendingLaunchQuery(appId: string): string {
