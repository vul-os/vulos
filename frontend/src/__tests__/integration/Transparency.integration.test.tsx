...
describe('TransparencyPanel — open + tier picker + export (integration)', () => {
  ...
  it('picking a new tier POSTs it to /api/assistant/tier', async () => {
    let tierBody: { tier: string } | null = null
    server.use(http.post('/api/assistant/tier', async ({ request }) => {
      const body: unknown = await request.json()
      if (typeof body !== 'object' || body === null || !('tier' in body) || typeof body.tier !== 'string') {
        throw new Error('expected a { tier: string } request body')
      }
      tierBody = { tier: body.tier }
