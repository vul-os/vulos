...
import WebPushToggle from '../core/notifiers/WebPushToggle.jsx'
import { probeState } from '../core/notifiers/webPushProbe.js'

const PUBKEY = 'BPabc-DEF_ghi'

function setSupport({ perm = 'granted', subscription = null } = {}) {
  ...
  globalThis.Notification = { permission: perm, requestPermission: async () => perm }
}
...
const okVapid = (enabled) => async () => ({ ok: true, json: async () => ({ enabled, publicKey: enabled ? PUBKEY : undefined }) })
...
describe('probeState', () => {
  it('unsupported when the browser lacks Web Push APIs', async () => {
    unsetSupport()
    expect(await probeState({ fetch: async () => ({ ok: true, json: async () => ({}) }) })).toBe('unsupported')
  })
  ...
  it('unconfigured when the vapid endpoint errors', async () => {
    expect(await probeState({ fetch: async () => ({ ok: false }) })).toBe('unconfigured')
  })
