/**
 * PeopleView — the address book itself: the PRIMARY view of the merged
 * contacts-and-calls surface (see Contacts.tsx, which is the app shell).
 *
 ...
 * connect).
 *
 * IT DOES NOT OWN THE PHONE. Calling arrives as a prop — the one CallSession the
 * shell holds — rather than a hook mounted per detail pane. Two reasons, both
 * of which were live defects: a hook here re-probed the modem on every contact
 * you clicked, and the "why you cannot call" explanation lived inside the detail
 * pane, so a box with no modem said nothing about it until you happened to open
 * a contact who had a phone number.
 *
 * THIS FILE MUST STAY USEFUL WITH NO TELEPHONY AT ALL. Most Vulos boxes have no
 * modem; an address book is not hardware-dependent and nothing here may become
 * conditional on a radio existing.
 */
...
export interface PeopleViewProps {
   ...
   * drags, and a viewport media query renders the two-pane desktop layout
   * inside a 380px rail on a 2560px monitor.
   */
  narrow: boolean
}
...
export default function PeopleView({ session, narrow }: PeopleViewProps) {
  ...
  // load() must NOT setState synchronously: the mount effect below calls it, and
      ...
      <div className="flex-1 min-w-0 flex flex-col bg-neutral-950">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain">
          {editing ? (
            ...
            <ContactDetail contact={selected} session={session} onEdit={() => startEdit(selected)}
...
function ContactDetail({ contact, session, onEdit, onDelete, saving }: ContactDetailProps) {
  const dialler = session
...
function PhoneRow({ number, dialler }: { number: string; dialler: CallSession }) {
