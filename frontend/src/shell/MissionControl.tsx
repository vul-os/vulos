...
export interface MissionControlCell { x: number; y: number; scale: number }
...
export function useMissionControlLayout(windows: ShellWindow[], isOpen: boolean): Record<number, MissionControlCell> {
  return useMemo(() => {
    ...
    const layout: Record<number, MissionControlCell> = {}
...
export default function MissionControl() {
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      if (target.closest('[data-window-id]') && !missionControlOpen) return
      if (target.closest('[data-no-pinch]')) return
