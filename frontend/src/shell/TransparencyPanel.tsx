...
import { useSovereignty, type SovereigntyEgress } from './sovereigntyTypes'
...
function useSealPolicy(open: boolean): boolean | null {
  const [sealDefault, setSealDefault] = useState<boolean | null>(null)
  useEffect(() => {
    if (!open) return
    let cancelled = false
...
function EgressBanner({ egress }: { egress: SovereigntyEgress }) {
...
function driveStorageRow(sealDefault: boolean | null): AtRestRow {
  if (sealDefault === true) {
    return {
...
function AtRestSection({ hasMasterKey, sealDefault }: { hasMasterKey: boolean | null; sealDefault: boolean | null }) {
...
function KeysSection({ hasMasterKey }: { hasMasterKey: boolean | null }) {
  return (
    <div>
...
export default function TransparencyPanel() {
  ...
  useEffect(() => {
    if (!panelOpen) return
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape') { e.stopPropagation(); closePanel() } }
