...
import type { ShellNotification, NotificationAction, ToastEntry, NotificationPrefs } from './notificationTypes'
...
// ── sound ────────────────────────────────────────────────────────────────────
function playChime(level: string) {
  try {
...
// ── toast card ───────────────────────────────────────────────────────────────
function ToastCard({ notif, onAction, onDismiss }: { notif: ShellNotification; onAction: (a: NotificationAction) => void; onDismiss: () => void }) {
...
// ── critical overlay ─────────────────────────────────────────────────────────
function CriticalOverlay({ notif, onAction, onDismiss }: { notif: ShellNotification; onAction: (a: NotificationAction) => void; onDismiss: () => void }) {
  const primary = notif.actions?.[0] || null
  return (
...
export default function Toasts() {
  const queue = useSyncExternalStore<ToastEntry[]>(subscribeToasts, getToasts as () => ToastEntry[])
  ...
  const chimedRef = useRef(new Set<string>())

  // CLUSTER-10: a 'sync' notification with a deep-link opens the ConflictResolver.
  const [resolverOpen, setResolverOpen] = useState(false)
  const seenSyncRef = useRef(new Set<string>())

  // Resolve a notification action through the shell's launch path.
  const runAction = useCallback((notif: ShellNotification, action: NotificationAction) => {
    markRead(notif.id)
    ...
    try {
      ...
      if (url && typeof url === 'string') {
        const intent = classifyIntent(url) as { url?: string }
  ...
  useEffect(() => {
    const timers = queue
      .filter(t => (t.notif.ttl ?? 0) > 0 && t.notif.level !== 'critical')
      .map(t => setTimeout(() => dismissToast(t.key), Math.min((t.notif.ttl ?? 0) * 1000, 30000)))
