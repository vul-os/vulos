...
export function useWindowShortcuts(): void {
  ...
  useEffect(() => {
    const arrowDir: Record<string, 'left' | 'right' | 'up' | 'down'> =
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (tileMod && arrowDir[e.key]) {
        if (!active) return
        e.preventDefault()
        // nextTile only returns null for a direction outside 'left'|'right'|
        // 'up'|'down' — unreachable here since arrowDir[e.key] already proved
        // e.key maps to one of those four.
      ...
      if (e.altKey && e.code === 'Backquote') {
        ...
        if (id != null) focusWindow(id as number)
        return
      }
