...
/** The subset of the shell's openWindow() that launchApp needs — a structural
 *  slice of ShellContextValue['openWindow'] (providers/ShellProvider.tsx),
 *  kept narrow so this module doesn't have to import the whole shell context
 *  just to describe what it calls. */
...
// legacy dispatch).
async function fetchLane(appId: string): Promise<string | null> {
  try {
...
function openInHostBrowser(url: string, title: string | undefined, icon: string | undefined, openWindow: LaunchAppDeps['openWindow']) {
 ...
 * Launchpad.launch.
 *
 * @param app   the registry app entry (from AppRegistry.getApps())
 * @param deps.openWindow  the shell's openWindow
 */
export async function launchApp(app: AppEntry | null | undefined, { openWindow }: LaunchAppDeps): Promise<void> {
  if (!app) return
