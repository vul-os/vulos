...
> **Status.** Partially shipped. What's real and wired: the `vk_` API-key auth layer (scoped to individual apps, issued per-user, revocable), the registry Ed25519 publisher-signing gate (REGISTRY-SIGN-01), and the one-click apt/Flatpak/download-and-extract install recipes with per-app subdomain + isolated namespace. **NOT built yet** (do not describe as shipped): the MCP platform (Model Context Protocol server — `internal/appsgate` reserves a `/mcp` prefix but registers no route), the `@vulos/apps` agent-operable surface, and `vat_` (Vulos App Token) scoped-capability tokens. See [`docs/APPS.md`](../docs/APPS.md) for the honest scope. Outstanding: build the agent/MCP surface; more curated registry entries; auto-gaming-mode flag.
>
...
### Forward plan: registry-as-feed (anti-rollback distribution) — phase 1 SHIPPED (publish-only)

`registry.json` is signed and distributed as a single object: the release key signs the file, and the release key is itself certified by the offline root key (root→release chain, see [KEY-CEREMONY.md](../docs/KEY-CEREMONY.md)). That proves *authenticity* (a box only accepts a registry the root-anchored chain vouches for) but not *freshness* — a box that only ever fetches "the current `registry.json`" has no built-in way to detect that it has been handed a stale, previously-superseded copy (a rollback/downgrade to a version with since-patched or since-revoked entries).

The fix is to **additionally** publish the registry as a **signed, append-only feed** per DMTAP-PUB §22.4 (the same feed primitive as the `substrate/` "Feeds & blobs" capability in `VULOS-PRODUCT-STANDARD.md`): each registry update becomes a new signed entry appended to the author's feed, chained to the previous entry, so a box can verify not just "this object is validly signed" but "this is the latest entry in a monotonic sequence I've verified from the start (or from my last-seen checkpoint)." A box that is shown an old entry as if it were current can detect the gap.

**Phase 1 (this wave): publish, don't consume yet.** Implemented in `backend/services/appnet/feed.go` — a JSON re-expression of dmtap §22.4.1's `FeedEntry`/`FeedHead` (sha256 in place of BLAKE3, Ed25519 as in the rest of this repo, JSON instead of CBOR to match `registry.json`'s own shape):

- `FeedEntry{seq, registry_hash, prev, ts}` — `registry_hash` is this repo's analogue of dmtap's `announce`: the sha256 content address of the `registry.json` bytes published at that position. `prev` is the entry's predecessor's content address; present on every entry except genesis (`seq=0`).
- `FeedHead{v, seq, tip, ts, signer, sig}` — only the head is signed; `tip`'s chain through every entry's `prev` transitively authenticates the whole log, exactly as dmtap specifies (no per-entry signature needed).
- Stored as one file, `registry-feed.json`, alongside `registry.json` (simpler than dmtap's per-object content-addressed store — appropriate for one small registry with one publisher).
- `make publish-feed` (→ `vulos-sign publish-feed`) appends a new signed entry after `make sign-registry`; it is a no-op if `registry.json` is unchanged since the last publish (mirrors dmtap §22.4.2's "re-presenting the current tip must be idempotent"). `make verify-feed` (→ `vulos-sign verify-feed`) walks the chain and checks the signed head against the anchor/release cert, with no private key — same posture as `verify-registry`.
- `registry-feed.json` is checked into the repo with a real genesis entry (`seq=0`), signed with the same dev release key every other entry in `registry.json` is signed with today; prod re-signing follows the same root-ceremony path as the rest of `registry.json` (docs/KEY-CEREMONY.md).
- Unit tests (`backend/services/appnet/feed_test.go`) cover genesis shape, monotonic `seq`+`prev` chaining, idempotent no-op republish, and fail-closed rejection of a broken chain, a non-monotonic `seq`, a genesis entry wrongly carrying `prev`, a tampered entry, and a wrong-key signature.

**Explicitly NOT done (later phase):** nothing at install time consults the feed yet — `LoadRegistry`, `VerifyEntrySignature`, and `resolveRegistryTrust` in `backend/services/appnet/registry.go` are unchanged, so a box that has never heard of `registry-feed.json` keeps working exactly as before (additive by construction). A future phase would have an installing box fetch/cache the feed head, apply §22.4.2's anti-rollback rule (reject a `seq` older than the highest it has ever accepted) against the `registry.json` it is about to trust, and detect a chain fork. `kerf-pub` (referenced in the DMTAP repo) remains the eventual reference implementation for a networked, multi-transport version of this same primitive if the feed ever needs to be fetched over anything other than "ships in the same repo as registry.json."
