...
> - **Instance↔instance hot path (code exists, non-functional in production):** `backend/services/sync/hotpath.go` implements the peering-mesh transport (SYNC-01/02) and its own tests pass, but it streams a `crsql_changes` virtual table that **only exists if cr-sqlite loaded** — which it never does under the no-CGO rule (`backend/services/store/store.go`'s `load_extension()` call cannot succeed against `modernc.org/sqlite`). Per hotpath.go's own doc comment: *"if not loaded, pushes are silently skipped."* So the hot path is dead code in every real deployment today, exercised only against a plain-SQLite emulation of the `crsql_changes` schema in tests.
...
|---|---|---|---|---|
| **Hot** | instance ↔ instance changeset streaming over the **peering mesh** (relay fallback for NAT / cross-location) | low | none (transient) | code exists (`hotpath.go`) but **non-functional in production** — depends on `crsql_changes`, which never populates (no cr-sqlite); see status above |
