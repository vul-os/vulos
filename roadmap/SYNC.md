...
> - **Wired end to end — `backend/cmd/server/crdtsync_wiring.go`,** called from `main.go` at the LAN mux site. The **`reminders`** table replicates between boxes: an ordinary SQL `INSERT`/`UPDATE`/`DELETE` on one box appears on another, with concurrent edits to *different columns of the same row* both surviving.
...
> - *Behavioural* — `TestStartCRDTSyncRegistersWorkingRoutes` runs the real wiring against a real mux and HTTP server and fails unless `/api/crdt/{pull,push,status}` are mounted **and** authenticated (401 without the secret, 200 with). Mutation-tested by dropping the registration, and separately by registering with a permissive authorizer.
