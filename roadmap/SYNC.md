...
| `app_registry` | no — **the refusal was wrong until 2026-08-16, and is now true** | The per-instance **realised** set: what each box actually managed to install. The refusal said "already replicated by `multiinstance/appsync` over the same fabric", and the reasoning that two engines converging one table would fight is sound — but the premise was false. **Nothing wrote `app_registry`**: `AppSync.LocalInstall`/`LocalUninstall` had no non-test caller anywhere under `backend/`, and `POST /api/store/install` went to `AppStore.Install`, which creates a directory and writes no row. Each engine deferred to the other and neither moved anything, while every component's own tests passed. Fixed by giving it a producer (`AppStore.RealisedVersions`/`Realise`/`Unrealise` satisfying `multiinstance.Realiser`) rather than by retiring appsync — see the next row and SYNC-INVENTORY.md §1. |
| `app_desired` | no | The fleet **desired** set: one row per app, no instance in the key — what the user asked to be installed everywhere. `app_registry` alone could not express it; an inventory records what a box *has*, which is a description, and "put it everywhere" is an intent. Carried by the **same** engine as its sibling, in one transaction off one changeset, so a box can never see a realisation report for an app whose desire has not arrived. Its algebra is deliberately not this engine's: removal is a **tombstone** that is never vacuumed (a deleted row is indistinguishable from "never wanted", so any peer still holding a pre-removal copy resurrects the app on every sync, forever), and an exact-timestamp tie breaks on the **actor id**, not "install wins" — two boxes acting on one user action stamp the same removal, and install-wins would mean it never lands. It is **not** quorum-gated, unlike the realised set: intent is expressed once, by a person, and requiring a majority of a user's own boxes to agree before an uninstall took effect is a product defect, not a hardening. Admission replaces it — an unverified origin's desire rows are **dropped**, not merely uncounted, because with no quorum to withhold from "does not count" would mean "applies". |
