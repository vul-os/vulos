...
**This is the one part of this document that is actually shipped** — not via cr-sqlite (per the reality check at the top), but via a real, pure-Go leaderless CRDT: `backend/internal/multiinstance/appsync.go` (LWW + OR-set + writer-node tie-break + signed uninstall quorum), transported same-LAN-only over mDNS + authenticated HTTPS by `backend/internal/fabric/`. a `reconcile.go` in `backend/services/cluster/` persisted the row set as JSON at the time of writing; that file no longer exists and the surviving app-state path is `backend/internal/multiinstance/appsync.go` above — this sentence is kept because the JSON-not-CRR distinction it draws is still the accurate one ("in production this lives in a cr-sqlite database; here it is persisted as JSON" — its own comment), not the SQL schema below, which remains the *design* shape:
