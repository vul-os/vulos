...
So that an **external router can route a user to the nearest healthy instance** of their cluster with **sticky-until-failure** affinity, the OSS/instance side provides exactly two hooks: it **exposes health** (the existing heartbeat + this `/api/health` endpoint) and it **supports session affinity** (sticky session; cross-instance session validity is the design intent of CLUSTER.md's structured-data sync, which is not yet shipped — see CLUSTER.md's reality check — so today session affinity should be treated as sticky-to-one-instance, not cluster-wide-valid). The routing decision itself is made by whatever router is in front of the cluster and is out of scope here; a self-hosted cluster with no router simply uses its configured connection mode above.
